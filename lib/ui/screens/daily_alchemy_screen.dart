  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Daily Alchemy', style: TextStyle(color: AppTheme.accentGold, fontFamily: 'Cinzel', fontSize: 24, fontWeight: FontWeight.bold)),
        centerTitle: true,
        leading: IconButton(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    isCompleted ? "TODAY'S ALCHEMY COMPLETE" : "TODAY'S CHALLENGE",
                    style: const TextStyle(color: AppTheme.textLight, fontSize: 20, fontWeight: FontWeight.bold, letterSpacing: 1.5),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    dateStr,
                    style: const TextStyle(color: AppTheme.accentGold, fontFamily: 'Cinzel', fontSize: 28, fontWeight: FontWeight.w900),
                  ),
                  const SizedBox(height: 32),
                  
                  if (isCompleted) ...[
                    _buildStatRow('Best Time', '${(record.bestDurationMs ?? 0) ~/ 1000}s'),
                    const SizedBox(height: 12),
                    _buildStatRow('Best Moves', '${record.bestMoveCount ?? 0}'),
                    const SizedBox(height: 12),
                    _buildStatRow('Stars', '${record.bestStars ?? 0}/3'),
                    const SizedBox(height: 12),
                    _buildStatRow('Reward Earned', '+${record.xpReward} XP', color: AppTheme.successGreen),
                    const SizedBox(height: 32),
                  ] else ...[
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: isCompleted ? AppTheme.surfaceDark : AppTheme.accentGold,
                      foregroundColor: isCompleted ? AppTheme.accentGold : AppTheme.backgroundDark,
                      padding: const EdgeInsets.symmetric(horizontal: 48, vertical: 16),
                      side: isCompleted ? const BorderSide(color: AppTheme.accentGold, width: 2) : null,
                  ...
                  if (isCompleted) ...[
                    const SizedBox(height: 16),
                    const Text('Practice grants 0 XP', style: TextStyle(color: Colors.white54, fontSize: 12)),
                  ],
                ],
              ),
            ),
  ...
  Widget _buildStatRow(String label, String value, {Color? color}) {
    return Row(
      ...
      children: [
        ...
        Text(value, style: TextStyle(color: color ?? AppTheme.accentGold, fontSize: 18, fontWeight: FontWeight.bold)),
      ],
    );
  }
}
